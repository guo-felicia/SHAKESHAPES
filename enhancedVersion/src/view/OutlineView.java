...
/**
 * Represents the class for interacting with the outline animation and viewing the graphic animation
 * of the model.
 */
public class OutlineView extends ControllableVisualView {
   ...
   */
  public OutlineView(IAnimation model, int speed) {
    super(model, speed);
   ...
   */
  public OutlineView(IAnimation model, int speed, boolean isStart, boolean isLooping) {
    super(model, speed);
    this.initialSpeed(speed);
    this.speed = speed;
    this.model = model;
   ...
   */
  public void fillOrOutline() {
    outlineButton.setActionCommand("Outline");
    outlineButton.addActionListener((ActionEvent e) -> {
      if (e.getActionCommand().equals("Outline")) {
        isOutline = true;
        outlineButton.setText("Fill");
        outlineButton.setActionCommand("Fill");
      } else {
        //do something
        isOutline = false;
  ...
  private void setShapes(List<IDrawableShape> loS, boolean outline) {
    animatePanel.setShapes(loS, isOutline);
  }
