...
/**
 * Represents the class for interacting with the discrete animation and viewing the graphic
 * animation of the model.
 */
public class DiscreteView extends ControllableVisualView {
   ...
   */
  public DiscreteView(IAnimation model, int speed) {
    super(model, speed);
   ...
   */
  public DiscreteView(IAnimation model, int speed, boolean isStart, boolean isLooping) {
    super(model, speed);
    this.initialSpeed(speed);
    this.speed = speed;
    this.model = model;
  ...
  private void setup() {
    ...
    //discreteButton
    discreteButton = new JButton("Continuous Animation");
    this.discreteOrContinues();

    speedPanel.add(speedUpButton);
    speedPanel.add(speedText);
    speedPanel.add(speedDownButton);
    speedPanel.add(discreteButton);
   ...
   */
  public void discreteOrContinues() {
    discreteButton.setActionCommand("Continuous");
    discreteButton.addActionListener((ActionEvent e) -> {
      if (e.getActionCommand().equals("Discrete")) {
        isDiscrete = true;
        discreteButton.setText("Continuous Animation");
        discreteButton.setActionCommand("Continuous");
      } else {
        //do something
        isDiscrete = false;
        discreteButton.setText("Discrete Animation");
  ...
  public void addActionListener(ActionListener actionListener) {
    ...
    discreteButton.addActionListener(actionListener);
  }
  ...
  private void discreteAnimate(int i) {
    List<Integer> loK = this.model.getKeys();
    if (i >= loK.size()) {
      checkLookHelper();
    } else {
      tick = loK.get(i);
