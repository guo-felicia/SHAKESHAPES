...
public class MergesortCreator {
   ...
   *
   * @param originalArr an array of integers to be sorted
   */
  public MergesortCreator(int[] originalArr) {
    this.originalArr = originalArr;
  }

  /**
   * Entry to the mergesort animation program.
   *
   * @param args arguments of the mergesort animation entry
   * @throws IOException if the animation cannot be created
   */
  public static void main(String[] args) throws IOException {
    int[] arr = {3, 44, 38, 5, 47, 15, 36, 26, 27, 2, 46, 4, 19, 50, 48};
    int[] originalArr = {3, 44, 38, 5, 47, 15, 36, 26, 27, 2, 46, 4, 19, 50, 48};
    int[] temp = new int[arr.length];
    MergesortCreator creator = new MergesortCreator(originalArr);
  ...
  private String motionToString(String name) {
    StringBuilder ans = new StringBuilder();
    Motion interval = model.getMotionsFor(name).get(0);
    String temp = "motion " + name + " " + interval.toString() + " " + interval.toString() + "\n";
    ans.append(temp);
